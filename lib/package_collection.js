...
PackageCollection = function(opts) {
    ...
    this.replaceDotToHypenOnFolderName = opts.replaceDotToHypenOnFolderName || false;
    ...
    collectPackages: function() {
        ...
        if (includeDev !== 'exclusive') {
            for (name in dependencies) {
                // replace all the '.' with '-' in the dependencie's folder name due to restriction not to have "." in folder names for navitaire environments.
                var packageFolderName = this.replaceDotToHypenOnFolderName ? name.replace(".", "-") : name;
                this.add(name, path.join(this.opts.paths.bowerDirectory, path.sep, packageFolderName));
            }
        }
